...
//        List<Integer> list = List.of(intArr); // 이런식으로 collection 변환해서 사용하지 않는 이유??
        ...
        System.out.println( Arrays.stream(intArr2).distinct().filter(e->e != null).reduce((e1,e2)->{return e1+e2;}));
        Arrays.stream(intArr2).distinct().filter(e->e != null).reduce((e1,e2)->{return e1+e2;});
        System.out.println();
...
//        System.out.println("퀴즈2 :" + strArr2ToInt);
        ...
        OptionalInt sum3 = Arrays.stream(strArr2)
                .filter((e)->e!=null)
                .mapToInt(Integer::parseInt) //매개변수를 함수가 실행하면서 사용할때 -> 컴파일러가 상상할 수 있을 만큼의 실행 = 매서드참조
...
//                .filter((e)->e!=null)
//                        .filter((e)->{!((Object)e instanceof Integer)})
