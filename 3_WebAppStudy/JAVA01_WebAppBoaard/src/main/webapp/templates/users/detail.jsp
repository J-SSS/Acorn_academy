...
<body>
    ...
    <main class="container">
        ...
        <p>상세주소 : <strong><%=user.getDetailAddress()%></strong></p>
        <p>등급(상태) : <strong><%=user.getPermission()%></strong></p>
        <p><a href="./update.do?u_id=<%=user.getUId()%>">수정폼</a></p>
        <%--
        시멘틱요소 : 검색엔진 로봇이 웹앱을 분석할 때 구조 및 정보를 파악하기 위해 태그를 사용하는 것
        프로그래머가 해당 태그의 이름으로 어떤 목적인지를 파악할 때 사용
        시각장애인이 해당 웹 앱을 이용할 때 말로 설명하는 것을 시멘틱요소를 이용한다
        b : 두껍게
        strong : 두껍게+강조
        div : 블럭
        nav : 블럭+네비게이션
