...
import com.acorn.s02_springboardstudy.service.BoardReplyService;
import lombok.Data;
    ...
    public @ResponseBody HandlerDto registerHandler(
        ...
        if(!img.isEmpty()){
            ...
            if(cotentTypes[0].equals("image")){
                ...
                reply.setImgPath("/static/public/img/reply/" +fileName);
            }
        }
        ...
        try {
            if(img!=null && !img.isEmpty()){//파일을 선택하지 않아도 null 이 아님
                ...
                if(contentTypes[0].equals("image")){
                    ...
                    reply.setImgPath("/static/public/img/reply/" +fileName); //서버가 이미지를 배포하는 위치
                }
            }
